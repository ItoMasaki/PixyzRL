...
import numpy as np
import torch
from distributions import Actor, Critic, Decoder, Encoder, Reward, Stochastic, Transition
...
from pixyzrl.policy import MPCPlanner
...
planner = MPCPlanner(action_size, planning_horizon, optimisation_iters, 1000, 100, transition, stochastic, reward_decoder)

step_loss = MaxLoss(KL(encoder, stochastic), ValueLoss(3.0)) - E(transition * encoder, LogProb(decoder)).mean() - E(transition * encoder, LogProb(reward_decoder)).mean()
iter_loss = IterativeLoss(step_loss=step_loss, series_var=["o_t", "a_t", "t_t", "r_t"], update_value={"z_tp1": "z_t"}).mean()


model = Model(iter_loss, distributions=[encoder, decoder, transition, stochastic, reward_decoder], optimizer=torch.optim.Adam, optimizer_params={"lr": 1e-3})
...
class MultiEnv:
    ...
    def step(self, actions):
        return zip(*[env.step(action) for env, action in zip(self.envs, actions, strict=False)], strict=False)
...
multi_env = MultiEnv("CarRacing-v2", 10)


for i in range(1, 1100001):
    with torch.no_grad():
        returns = multi_env.reset()
        observations = torch.tensor([obs.transpose(2, 0, 1).reshape(3, 96, 96) / 255 for obs, info in returns], dtype=torch.float32).to("mps")
        z_t = torch.zeros(10, belief_size).to("mps")
        s_t = encoder.sample({"o_t": observations, "z_t": z_t})["s_t"]

        for step in range(1000):
            action = policy_old.sample({"z_t": z_t, "s_t": s_t})["a_t"].detach().cpu().numpy().astype(np.float64)
            returns = multi_env.step(action)  # Zip of (observation, reward, terminated, info)

            # Unzip the returns
            observations, reward, terminated, truncated, info = [*returns]

            # Convert the observations to tensor
            obs = torch.tensor([obs.transpose(2, 0, 1).reshape(3, 96, 96) / 255 for obs in observations], dtype=torch.float32).to("mps")
            act = torch.tensor(action, dtype=torch.float32).to("mps")
            ...
            o_t = decoder.sample_mean({"z_t": z_t, "s_t": s_t}).detach().cpu().numpy().reshape(10, 3, 96, 96).transpose(0, 2, 3, 1)
            r_t = reward_decoder.sample_mean({"z_t": z_t, "s_t": s_t}).detach().cpu().numpy().reshape(10)
            z_t = z_tp1.detach()

            print(step, reward)

        #     memory.add(observations, action, rewards, terminates)

        #     if all(terminated for obs, rew, terminated, info in returns):
        #         break
...
#                         break
#
#                     memory.add(observation / 255, action, reward, True)
#
#                     obs = torch.tensor(observation.transpose(2, 0, 1).reshape(1, 3, 96, 96) / 255, dtype=torch.float32).to("mps")
...
#
#             loss, return_dict = model.train({"o_t": obs, "a_t": act, "z_t": z_t, "t_t": trm, "r_t": rwd}, return_dict=True)
#
...
#
#             adv, returns = advantage_and_return(imged_reward, value_pred, 0.99, 0.95)
#
#             # Advantage estimation
#             ppo_loss, _ = ppo(policy, policy_old, 0.15).mean()({"z_t": actor_beliefs, "s_t": actor_states, "a_t": imged_actions, "A": adv.detach()})
#
